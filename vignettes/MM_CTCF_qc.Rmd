...
```{r clustering, fig.width=16, fig.height=8}

prof_dt = ssvFetchBigwig(bw_config_dt, asses_gr.c, return_data.table = TRUE, n_region_splits = 20)
clust_dt = ssvSignalClustering(prof_dt, nclust = 6, max_cols = Inf)
assign_dt = make_assign_dt(clust_dt)

signal_plots = plot_signals(prof_dt, asses_gr.c, assign_dt = assign_dt, frip_dt = frip_dt, scc_dt = scc_dt, anno_grs = anno_grs)
signal_plots[-1]
```
